...
const DEFAULT_SETTINGS = {
  ...
  api_base_url: "https://mushu.space",
  supabase_url: "",
  supabase_anon_key: "",
...
async function refreshAccountFromBackend() {
  ...
  let response = await fetchWithBackendAuth(`${apiBaseUrl}/api/me`, session.access_token, {
    cache: "no-store",
  });
  ...
  if (response.status === 401) {
    ...
    response = await fetchWithBackendAuth(`${apiBaseUrl}/api/me`, session.access_token, {
      cache: "no-store",
    });
  }
...
async function translateText(rawText, targetLanguage) {
  ...
  const data = await callMushuJson("/api/translate", {
    text,
...
async function transcribeAudio(audioChunks, mimeType) {
  ...
  let response = await fetchWithBackendAuth(`${apiBaseUrl}/api/transcribe`, accessToken, {
    method: "POST",
    headers: {
  ...
  ipcMain.handle("mushu:invoke", async (event, command, args = {}) => {
    ...
    switch (command) {
      ...
      case "explain_stream":
        ...
        const explainData = await callMushuJson("/api/agent", {
